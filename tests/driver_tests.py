...
from tpot.driver import positive_integer, float_range, _get_arg_parser, _print_args, _read_data_file, \
	load_scoring_function, tpot_driver
...
def test_scoring_function_argument():
	with captured_output() as (out, err):
		...
		assert_equal(load_scoring_function('driver_tests.test_scoring_function_argument').__name__,
...
class ParserTest(TestCase):
	...
	def test_default_param(self):
		...
		self.assertEqual(args.TARGET_NAMES, ['class'])
		self.assertEqual(args.TPOT_MODE, 'classification')
		self.assertEqual(args.VERBOSITY, 1)
		self.assertEqual(args.MULTI_OUTPUT, False)

...
from tpot.gp_deap import mutNodeReplacement, _wrapped_cross_val_score, pick_two_individuals_eligible_for_crossover, \
	cxOnePoint, varOr, initialize_stats_dict
...
def test_init_default_scoring_2():
	...
	assert "This scoring type was deprecated" in str(
		w[-1].message)  # deap 1.2.2 warning message made this unit test failed
...
def test_sample_weight_func():
	...
	np.random.seed(42)
	cv_score1 = cross_val_score(tpot_obj.fitted_pipeline_, training_features_r, training_target_r, cv=3,
								scoring='neg_mean_squared_error')

	np.random.seed(42)
	cv_score2 = cross_val_score(tpot_obj.fitted_pipeline_, training_features_r, training_target_r, cv=3,
								scoring='neg_mean_squared_error')

	np.random.seed(42)
	cv_score_weight = cross_val_score(tpot_obj.fitted_pipeline_, training_features_r, training_target_r, cv=3,
									  scoring='neg_mean_squared_error', fit_params=training_target_r_weight_dict)
...
def test_evaluated_individuals_():
	...
	for pipeline_string in sorted(tpot_obj.evaluated_individuals_.keys()):
		...
		try:
			cv_scores = cross_val_score(sklearn_pipeline, training_features, training_target, cv=5, scoring='accuracy',
										verbose=0)

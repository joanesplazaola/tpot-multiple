...
from .builtins import CombineDFs, StackingEstimator, Mode
...
class TPOTBase(BaseEstimator):
	...
	def __init__(self, generations=100, population_size=100, offspring_size=None,
		...
		multi_output: bool, optional (default: False)
			Flag indicating whether the target has multiple dimensions.
		...
		if scoring:
			if isinstance(scoring, str):
				...
				if args_list == ["y_true", "y_pred"] or (hasattr(module, 'startswith') and \
														 (module.startswith('sklearn.metrics.') or module.startswith(
															 'tpot.metrics')) and \
	...
	def fit(self, features, target=None, sample_weight=None, groups=None, features_valid=None, target_valid=None):
		...
		if self.subsample < 1.0:
			features, _, target, _ = train_test_split(features, target, train_size=self.subsample,
	...
	def predict(self, features):
		...
		features = self._check_dataset(features, target=None, sample_weight=None, multi_output=self.multi_output)
	...
	def _save_periodic_pipeline(self):
		try:
			...
			filename = os.path.join(self.periodic_checkpoint_folder,
									'pipeline_{}.py'.format(datetime.now().strftime('%Y.%m.%d_%H-%M-%S')))
	...
	def _create_periodic_checkpoint_folder(self):
		try:
			...
			self._update_pbar(
				pbar_msg='Created new folder to save periodic pipeline: {}'.format(self.periodic_checkpoint_folder))
		except OSError as e:
	...
	def export(self, output_file_name, skip_if_repeated=False):
		...
		to_write = export_pipeline(self._optimized_pipeline, self.operators, self._pset, self.mode, self._imputed,
								   self._optimized_pipeline_score, self.random_state, self.multi_output)
	...
	def _check_dataset(self, features, target, sample_weight=None, multi_output=False):
		...
		features: array-like {n_samples, n_features}
			Feature matrix
		target: array-like {n_samples, n_targets} or None
			List/matrix of class labels for prediction
		sample_weight: array-like {n_samples} (optional)
			List of weights indicating relative importance
		multi_output: bool
			Tells whether the target is multi dimensional or just a list.
		Returns
		-------
		(features, target)
		"""
		...
		try:
			if target is not None:
				X, y = check_X_y(features, target, accept_sparse=True, dtype=np.float64, multi_output=multi_output)
				return X, y
			else:
	...
	def _compile_to_sklearn(self, expr):
		...
		"""
		sklearn_pipeline_str = generate_pipeline_code(expr_to_tree(expr, self._pset), self.operators, self.mode,
													  self.multi_output, )
	...
	def _update_evaluated_individuals_(self, result_score_list, eval_individuals_str, operator_counts, stats_dicts):
		...
		for result_score, individual_str in zip(result_score_list, eval_individuals_str):
			if type(result_score) in [float, np.float64, np.float32]:
				self.evaluated_individuals_[individual_str] = self._combine_individual_stats(

...
def export_pipeline(exported_pipeline, operators, pset, mode=None, impute=False, pipeline_score=None,
					random_state=None, multi_output=False, ):
	...
	pipeline_text = generate_import_code(exported_pipeline, operators, mode, impute, multi_output)
	pipeline_code = pipeline_code_wrapper(
		generate_pipeline_code(pipeline_tree, operators, mode, multi_output, export=True))
...
def generate_import_code(pipeline, operators, mode=None, impute=False, multi_output=False):
...
def generate_pipeline_code(pipeline_tree, operators, mode=None, multi_output=False, export=False):
	...
	if multi_output:
		steps[-1] = 'MultiOutput{}({})'.format(mode.name.capitalize(), steps[-1])
	# number of steps in a pipeline
	num_step = len(steps)
	if num_step > 1 or not export:
